	...
	function SendVisitorData(Socket, $state) {
		...
		function send(form, lastActiveIndex, timeElapsed) {
			...
			var visitorData = {
				...
				formId: form._id,
				lastActiveField: form.form_fields[lastActiveIndex].globalId,
    ...
    function ($http, TimeCounter, $filter, $rootScope, SendVisitorData, $translate, $timeout) {
        return {
            ...
            controller: ["$document", "$window", "$scope", function($document, $window, $scope){
		        $scope.disableScrollWatch = false;
                ...
                $scope.reloadForm = function(){
                    ...
                    $scope.setActiveField($scope.myform.visible_form_fields[0].globalId, 0, false);
                ...
                */
				...
				var getActiveField = function(){
					...
					if($scope.selected.globalId === FORM_ACTION_ID) {
				...
				$scope.isActiveField = function(field){
					if($scope.selected.globalId === field.globalId) {
						return true
					}
					return false;
				};
                ...
                $scope.setActiveField = $rootScope.setActiveField = function(field_id, field_index, animateScroll) {
	    			...
	    			if(!field_id){
	    				field_id = $scope.myform.visible_form_fields[field_index].globalId;
					...
					if($scope.selected.globalId === field_id){
						return;
		    		}

                    $scope.selected.globalId = field_id;
                    ...
                    if(animateScroll){
                        $scope.disableScrollWatch = true;
                        setTimeout(function() {
                            $document.scrollToElement(angular.element('#'+$scope.selected.globalId), -10, 200).then(function() {
								$scope.disableScrollWatch  = false;
								setTimeout(function() {
                ...
                $rootScope.nextField = $scope.nextField = function(){
					if($scope.selected && $scope.selected.index > -1){
						...
						if($scope.selected.globalId !== FORM_ACTION_ID){
							...
							if(currField.logicJump.enabled && currField.logicJump.jumpTo && evaluateLogicJump(currField)){
                ...
                */
                $scope.exitStartPage = function(){
                    ...
                    if($scope.myform.visible_form_fields.length > 0){
                        $scope.selected.globalId = $scope.myform.visible_form_fields[0].globalId;
                    }
                };
