...
angular.module('TellForm.templates', []).run(["$templateCache", function ($templateCache) {
  ...
  $templateCache.put("modules/users/views/password/reset-password-invalid.client.view.html",
    "<section class=\"row auth\" data-ng-controller=PasswordController><div class=\"row valign\"><div class=\"col-md-4 col-md-offset-4\"><div class=\"col-md-12 text-center\" style=\"padding-bottom: 50px\"><img src=/static/modules/core/img/logo_white.svg height=100px></div><h3 class=\"col-md-12 text-center\">{{ 'PASSWORD_RESET_INVALID' | translate }}</h3><div class=\"col-md-12 text-center\"><a class=\"btn btn-signup btn-rounded btn-block\" href=/#!/password/forgot>{{ 'ASK_FOR_NEW_PASSWORD' | translate }}</a></div></div></div></section>");
...
// Setting up route
angular.module('core').config(["$stateProvider", "$urlRouterProvider", "Authorization", function($stateProvider, $urlRouterProvider, Authorization) {
		// Redirect to home view when route not found
		$urlRouterProvider.otherwise('/forms');
	}
]);

angular.module(ApplicationConfiguration.applicationModuleName).run(["$rootScope", "Auth", "$state", "$stateParams", function($rootScope, Auth, $state, $stateParams) {
...
angular.module(ApplicationConfiguration.applicationModuleName).run(["$rootScope", "Auth", "User", "Authorizer", "$state", "$stateParams", function($rootScope, Auth, User, Authorizer, $state, $stateParams) {
...
angular.module('core').controller('HeaderController', ["$rootScope", "$scope", "Menus", "$state", "Auth", "User", "$window", "$translate", function ($rootScope, $scope, Menus, $state, Auth, User, $window, $translate) {
...
angular.module('forms').run(["Menus", function(Menus) {
...
// Setting up route
angular.module('forms').config(["$stateProvider", function($stateProvider) {
		// Forms state routing
		$stateProvider.
		state('listForms', {
			url: '/forms',
...
angular.module('forms').factory('GetForms', ["$resource", "FORM_URL", function($resource, FORM_URL) {
...
angular.module('users').controller('AuthenticationController', ["$scope", "$location", "$state", "$rootScope", "User", "Auth", "$translate", "$window", function($scope, $location, $state, $rootScope, User, Auth, $translate, $window) {
...
angular.module('users').controller('PasswordController', ["$scope", "$stateParams", "$state", "User", "$translate", "$window", function($scope, $stateParams, $state, User, $translate, $window) {
...
angular.module('users').controller('SettingsController', ["$scope", "$rootScope", "$http", "$state", "Users", "Auth", function($scope, $rootScope, $http, $state, Users, Auth) {
...
angular.module('users').controller('VerifyController', ["$scope", "$state", "$rootScope", "User", "Auth", "$stateParams", "$translate", "$window", function($scope, $state, $rootScope, User, Auth, $stateParams, $translate, $window) {
		$translate.use($window.locale);
...
angular.module('users').factory('Auth', ["$window", function($window) {
...
angular.module('users').factory('User', ["$window", "$q", "$timeout", "$http", "$state", function($window, $q, $timeout, $http, $state) {
...
// Forms controller
angular.module('forms').controller('AdminFormController', ["$rootScope", "$window", "$scope", "$stateParams", "$state", "Forms", "CurrentForm", "$http", "$uibModal", "myForm", "$filter", "$translate", function($rootScope, $window, $scope, $stateParams, $state, Forms, CurrentForm, $http, $uibModal, myForm, $filter, $translate) {
...
// Forms controller
angular.module('forms').controller('ListFormsController', ["$rootScope", "$scope", "$stateParams", "$state", "GetForms", "CurrentForm", "$http", "$uibModal", "myForms", function($rootScope, $scope, $stateParams, $state, GetForms, CurrentForm, $http, $uibModal, myForms) {
...
angular.module('forms').directive('configureFormDirective', ["$rootScope", "$http", "Upload", "CurrentForm", function ($rootScope, $http, Upload, CurrentForm) {
        return {
...
angular.module('forms').directive('editFormDirective', ["$rootScope", "FormFields", "$uibModal", function ($rootScope, FormFields, $uibModal) {
        return {
...
angular.module('forms').directive('editSubmissionsFormDirective', ["$rootScope", "$http", "Forms", "$stateParams", "$interval", function ($rootScope, $http, Forms, $stateParams, $interval) {
        return {
            ...
            controller: ["$scope", function($scope){
                ...
                $scope.getVisitors = function(){
                    $http({
                      ...
                      url: '/forms/'+$scope.myform._id+'/visitors'
                        ...
                        $scope.analyticsData.deviceStatistics = $scope.analyticsData.deviceStatistics[0];
                    });
                };
                ...
                */
                $scope.DeviceStatistics = (function(){
                    ...
                    if($scope.analyticsData.deviceStatistics.length){
                        for(var i=0; i<$scope.analyticsData.deviceStatistics.length; i++){
                            var currDevice = $scope.analyticsData.deviceStatistics[i];
...
angular.module('forms').service('FormFields', ["$rootScope", "$translate", "Auth", function($rootScope, $translate, Auth) {
...
angular.module('forms').factory('Submissions', ["$resource", function($resource) {
